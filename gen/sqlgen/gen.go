...
func (g *Gen) GenerateCommon() {
	...
	p := g
	p.NewImport("sq", "github.com/ng-vu/sqlgen/sq")()
...
func (g *Gen) generateGoderive(def *typeDef) {
	gg, ok := g.Interface.(ggen.Goderive)
	if !ok {
		return
	}
	...
	if len(def.joins) == 0 {
		g.P("")
		g.P(`// Type %v represents table "%v"`, pStr, tableName)
		g.P("func %v(_ %v%v) bool { return true }", gg.GetFuncName(typ), pStr, extra)
		...
		for i, join := range def.joins {
			extra += fmt.Sprintf(
				...
				i, g.TypeString(join.joinTyp), i, i)
		...
		g.P("")
		g.P(`// Type %v represents a join`, pStr)
		g.P("func %v(_ %v%v) bool {", gg.GetFuncName(typ), pStr, extra)
		g.In()
		g.P("%v = []sq.JOIN_TYPE{%v}", _JoinTypes, strings.Join(joinTypes, ","))
		g.P(`%v = as + "."`, _As)
		g.P("%v = []sq.AS{%v}", _JoinAs, strings.Join(joinAs, ","))
		g.P("%v = []string{%v}",
			_JoinConds, strings.Join(joinConds, ","))
		g.P("%v = (%v)(nil).__sqlSelect(make([]byte, 0, 1024))",
			_Select, pStr)
		g.P("%v = (%v)(nil).__sqlJoin(make([]byte, 0, 1024), %v)",
...
func parseColumnsFromType(g *Gen, path pathElems, root types.Type, sTyp *types.Struct) ([]*colDef, error) {
	var cols []*colDef
	for i, n := 0, sTyp.NumFields(); i < n; i++ {
		...
		if inline {
			...
			if t, ok := typ.Underlying().(*types.Struct); ok {
				inlineCols, err := parseColumnsFromType(g, fieldPath, root, t)
				if err != nil {
					return nil, err
				}
